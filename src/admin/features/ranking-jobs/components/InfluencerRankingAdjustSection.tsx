...
import { INITIAL_INFLUENCER_RANKING, type InfluencerRankingRow } from "../data/mockRanking";
...
export function InfluencerRankingAdjustSection() {
  const [rows, setRows] = useState<InfluencerRankingRow[]>(INITIAL_INFLUENCER_RANKING);
  const [savedOrder, setSavedOrder] = useState(() =>
    orderSignature(INITIAL_INFLUENCER_RANKING),
  );
  ...
  function handleSave() {
    console.log("[ranking] influencer order save", rows);
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-run-muted">
        up/down 집계 외 관리자가 순위를 수동 조정합니다. 변경 후 저장해야 반영됩니다.
      </p>
        ...
        <table className="admin-table w-full table-fixed border-collapse text-sm">
          ...
          <thead>
            <tr className="admin-table-head border-b border-run-border text-left text-xs font-medium uppercase tracking-wider text-run-muted">
              ...
              <th className="py-3 pr-4 whitespace-nowrap">인플루언서명</th>
              <th className="py-3 pr-4 whitespace-nowrap">플랫폼</th>
              <th className="py-3 pr-4 whitespace-nowrap">up 수</th>
              <th className="py-3 pr-4 whitespace-nowrap">down 수</th>
            ...
            {rows.map((row, index) => (
                ...
                >
                  ...
                  <td className="py-3.5 pr-4">
                    <span className={adminPlatformPillBadge[row.platform]}>{row.platform}</span>
                  </td>
                  <td className="py-3.5 pr-4 text-white">{row.upCount.toLocaleString()}</td>
                  <td className="py-3.5 pr-4 text-run-muted">{row.downCount.toLocaleString()}</td>
